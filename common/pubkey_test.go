...
func TestPubKey_GetAddress(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "test1",
			key:     "0231a696329ae11d4af44bd3a6ca57b643cb5cb6308cc7472c71dcc7aa5390a942",
			chain:   XRPChain,
			want:    "rDcxBkoQjAYR9CLV2NFpMnGcRXfWRDoVQ",
			wantErr: false,
		},
		{
			name:    "test1",
			key:     "0231a696329ae11d4af44bd3a6ca57b643cb5cb6308cc7472c71dcc7aa5390a942",
			chain:   ETHChain,
			want:    "0x73635dcEc8A5aE94a98559C3B4fe8078e253EDF6",
			wantErr: false,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			p, err := NewPubKey(tt.key)
			if err != nil {
				t.Fatalf("could not construct receiver type: %v", err)
			}
			got, gotErr := p.GetAddress(tt.chain)
			if gotErr != nil {
				if !tt.wantErr {
					t.Errorf("GetAddress() failed: %v", gotErr)
				}
				return
			}
			if tt.wantErr {
				t.Fatal("GetAddress() succeeded unexpectedly")
			}
