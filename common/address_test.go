...
func (s *AddressSuite) TestAddress(c *C) {
	...
	// mainnet p2pk compressed (0x03), UTXO SignTx unable to sign for (not a THORChain-supported format)
	_, err = NewAddress("03b0bd634234abbb1ba1e986e884185c61cf43e001f9137f23c2c409273eb16e65")
	c.Check(err, NotNil)

	// mainnet p2pk uncompressed (0x04), UTXO SignTx unable to sign for (not a THORChain-supported format)
	...
	// mainnet p2pk hybrid (0x06), UTXO SignTx unable to sign for (not a THORChain-supported format)
	...
	// mainnet p2pk hybrid (0x07), UTXO SignTx unable to sign for (not a THORChain-supported format)
	...
	// mocknet p2pk compressed (0x02), UTXO SignTx unable to sign for (not a THORChain-supported format)
	_, err = NewAddress("02192d74d0cb94344c9569c2e77901573d8d7903c3ebec3a957724895dca52c6b4")
	c.Check(err, NotNil)
	...
	c.Check(err, NotNil)
	// This Mainnet address is not valid for Mocknet(/Testnet).
	...
	c.Check(err, IsNil)
	// XRP address that decodes, but has incorrect checksum
	addr, err = NewAddress("r4KYDZBbcAaJJ5MQPwRR9apJ2p5EBz8bq")
	c.Check(err, NotNil)
