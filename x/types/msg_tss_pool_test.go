...
func (s *MsgTssPoolSuite) TestMsgTssPool(c *C) {
	...
	m, err := NewMsgTssPool(pks, pk, nil, nil, KeygenType_AsgardKeygen, 1, Blame{}, chains, nil, keygenTime)
	c.Assert(m, NotNil)
	c.Assert(err, IsNil)
	c.Assert(m.ValidateBasic(), NotNil)
	m, err = NewMsgTssPool(nil, pk, nil, nil, KeygenType_AsgardKeygen, 1, Blame{}, chains, addr, keygenTime)
	c.Assert(m, NotNil)
	c.Assert(err, IsNil)
	c.Assert(m.ValidateBasic(), NotNil)
	m, err = NewMsgTssPool(pks, "", nil, nil, KeygenType_AsgardKeygen, 1, Blame{}, chains, addr, keygenTime)
	c.Assert(m, NotNil)
	c.Assert(err, IsNil)
	c.Assert(m.ValidateBasic(), NotNil)
	m, err = NewMsgTssPool(pks, "bogusPubkey", nil, nil, KeygenType_AsgardKeygen, 1, Blame{}, chains, addr, keygenTime)
	c.Assert(m, NotNil)
	c.Assert(err, IsNil)
	c.Assert(m.ValidateBasic(), NotNil)

	// fails on empty chain list
	msg, err = NewMsgTssPool(pks, pk, nil, nil, KeygenType_AsgardKeygen, 1, Blame{}, []string{}, addr, keygenTime)
	c.Assert(err, IsNil)
	c.Check(msg.ValidateBasic(), NotNil)
	// fails on duplicates in chain list
	msg, err = NewMsgTssPool(pks, pk, nil, nil, KeygenType_AsgardKeygen, 1, Blame{}, []string{common.RuneAsset().Chain.String(), common.RuneAsset().Chain.String()}, addr, keygenTime)
	c.Assert(err, IsNil)
	c.Check(msg.ValidateBasic(), NotNil)

	msg1, err := NewMsgTssPool(pks, pk, nil, nil, KeygenType_AsgardKeygen, 1, Blame{}, chains, addr, keygenTime)
	c.Assert(err, IsNil)
	msg1.ID = ""
	...
	msg2, err := NewMsgTssPool(append(pks, ""), pk, nil, nil, KeygenType_AsgardKeygen, 1, Blame{}, chains, addr, keygenTime)
	c.Assert(err, IsNil)
