...
import (
	...
	"github.com/mapprotocol/compass-tss/pkg/chainclients/shared/payload"
...
func NewXrpBlockScanner(rpcHost string,
	...
	return &XrpBlockScanner{
		...
		bridge:           bridge,
		solvencyReporter: solvencyReporter,
		payloadEncoder:   payload.New(bridge),
	}, nil
}
...
func (c *XrpBlockScanner) processTxs(height int64, rawTxs []transaction.FlatTransaction) ([]*types.TxInItem, error) {
	...
	for _, rawTx := range rawTxs {
		...
		if len(memo) == 0 {
			ctxLog.Msg("skipping tx with empty memo")
			continue
		}
		...
		if isSender { // sender
			...
			case mem.TxInbound:
				...
				toBytes = xrp.DecodeBase58(payment.Destination.String())
			case mem.TxMigrate:
				toBytes = []byte{}
			...
			toToken, err = c.bridge.GetTokenAddress(selfId, common.XRPAsset.Symbol.String())
			if err != nil {
				return nil, fmt.Errorf("fail to get token address for XRP: %w", err)
			}
		}
		// xrp2other
		if isTo {
			...
			if invalidMemo {
				...
				case mem.TxOutbound:
					...
					payload, err = c.payloadEncoder.Encode(nativeToken.Symbol.String(),
		...
		c.logger.Info().Str("txid", hash).Msg("find tx")
	}
