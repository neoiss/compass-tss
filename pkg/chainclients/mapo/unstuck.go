...
func (b *Bridge) unstuck() {
	b.logger.Info().Msg("starting unstuck routine")
	defer b.logger.Info().Msg("stopping unstuck routine")
	defer b.wg.Done()

	for {
		select {
		case <-b.stopChan: // exit when stopChan is closed
			return
...
func (b *Bridge) unstuckAction() {
	height, err := b.GetBlockHeight()
	if err != nil {
		b.logger.Err(err).Msg("failed to get THORChain block height")
		return
	}
	...
	signedTxItems, err := b.blockScanner.blockMetaAccessor.GetSignedTxItems()
	if err != nil {
		b.logger.Err(err).Msg("failed to get all signed tx items")
		return
	}
	for _, item := range signedTxItems {
		...
		if item.Height > height {
			clog.Warn().Msg("signed outbound height greater than current mapBridge height")
			continue
		}
		...
		if err = b.blockScanner.blockMetaAccessor.RemoveSignedTxItem(item.Hash); err != nil {
...
func (b *Bridge) unstuckTx(clog zerolog.Logger, item evmtypes.SignedTxItem) error {
	ctx, cancel := b.getTimeoutContext()
	defer cancel()
	tx, pending, err := b.ethClient.TransactionByHash(ctx, ecommon.HexToHash(item.Hash))
	if err != nil {
	...
	address, err := pubKey.GetAddress(b.cfg.ChainID)
	if err != nil {
	...
	currentGasRate := big.NewInt(1).Mul(b.GetGasPrice(), big.NewInt(2))
	...
	tipCap := new(big.Int).Mul(currentGasRate, big.NewInt(int64(10))) // todo handler add cfg
	...
	cancelTx := etypes.NewTx(&etypes.DynamicFeeTx{
		...
		To:        &to,
		Value:     big.NewInt(0),
		Gas:       b.blockScanner.cfg.MaxGasLimit,
		GasFeeCap: currentGasRate,
		GasTipCap: tipCap,
	})

	// handler need sign
	rawBytes, err := b.kw.LocalSign(tx)
	if err != nil {
		clog.Err(err).Msg("fail to sign tx")
		return nil
	}
	...
	ctx, cancel = b.getTimeoutContext()
	defer cancel()
	err = b.ethClient.SendTransaction(ctx, cancelTx)
...
func (b *Bridge) AddSignedTxItem(hash string, height int64, vaultPubKey string, toi *stypes.TxOutItem) error {
	return b.blockScanner.blockMetaAccessor.AddSignedTxItem(evmtypes.SignedTxItem{
		Hash:        hash,
		Height:      height,
