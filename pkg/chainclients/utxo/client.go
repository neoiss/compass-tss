...
func (c *Client) Start(
	...
	go runners.SolvencyCheckRunner(
		c.GetChain(), c, c.bridge, c.stopchan, c.wg, constants.MAPRelayChainBlockTime,
	)
}
...
func (c *Client) GetAccount(pubkey common.PubKey, height *big.Int) (common.Account, error) {
	...
	for _, item := range utxos {
		...
		if item.Confirmations == 0 {
			// pending tx in mempool, only count sends from vault
...
// ReportSolvency reports solvency for all vault vaults.
