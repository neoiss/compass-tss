...
func (c *Client) getTxIn(tx *btcjson.TxRawResult, height int64, isMemPool bool, vinZeroTxs map[string]*btcjson.TxRawResult) (types.TxInItem, error) {
	processor := NewTxProcessor(c)
	ctx, err := processor.PrepareContext(tx, height, isMemPool, vinZeroTxs)
	if err != nil {
	...
	if c.isAsgardAddress(ctx.Sender) {
		return processor.ProcessAsgardSenderTx(ctx)
	}

	if c.isAsgardAddress(ctx.Receiver) {
		return processor.ProcessAsgardReceiverTx(ctx)
	}
